...
public class CikServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (req.getServerName().contains("edgarwrap.appspot.com")) {
			try {
				URI re = new URI("http://edgarwrap.ontologycentral.com/"
						+ req.getRequestURI());
				re = re.normalize();
		...
		// Fix error: The method setCharacterEncoding(String) is undefined for
		// the type HttpServletResponse
		...
		int index = id.indexOf("/cik/");
		id = id.substring(index + "/cik/".length());
		...
		try {
			...
			XMLOutputFactory factory = (XMLOutputFactory) ctx
					.getAttribute(Listener.FACTORY);
			...
			ch.writeAttribute("rdf:resource",
					"http://cbasewrap.ontologycentral.com/company/ontologycentral#id");
			ch.writeEndElement();
